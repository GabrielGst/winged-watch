...
def retrieve_forecast(parameters, filename):
  ...
  if os.path.exists(filename):
    ...
    if current_day - file_day < 1:
      log(f"Response [{current_day - file_day}]. The weather dataset file time {file_time_converted} and current time {current_time} are on the same day. No download needed.")
 
    else:
      log(f"Response [{current_day - file_day}]. The weather dataset file time {file_time_converted} and current time {current_time} are on different days. Proceeding with download.")
      ...
      log(f"Latest forecast available : {time}")
      ...
      log(f"Forecast downloaded : {result}")
    ...
    log(f"The weather dataset file does not exist yet. Proceding with download.")
...
def process_forecast(filename, export=False):
  ...
  log("Proceeding with grib to json export.")
  ds = xr.open_dataset(filename, engine="cfgrib", decode_times=False) # To avoid datetime incompatibility use decode_times=False kwarg in xarray.open_dataset
  ...
  if export:
    ...
    log(f"Wind data exported to ./public/wind.json")
      
  else:
    log("Wind data not exported.")


def test():
  """Test function to check if the retrieve and process functions are working as expected.
  """
...
def main(force_export=False):
  """Main function to retrieve and process the forecast data.

  Args:
      force_export (bool, optional): _description_. Defaults to False.
  """
  log(f"Retrieving and processing forecast data. Export: {force_export}")
  
  parameters = ['msl', '10u', '10v']
  filename = 'medium-wind-10m.grib'
  
  filename, exp = retrieve_forecast(parameters, filename)
  exportBool = force_export or exp
  process_forecast(filename, export=exportBool)
